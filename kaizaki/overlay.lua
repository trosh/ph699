...
local l1 = 10 -- moyenne sur combien de points (a gauche et a droite)
...
local sizex, sizey = 800, 300
local image = gd.create(sizex, sizey)
image:gifAnimBegin(output, false, 0)
for frm = 2, 196, 2 do --2, 46, 3 do
	...
	local image = gd.createFromPng("diffs/diff_"..frm..".png")
	local white = image:colorResolve(255, 255, 255)
	local white50 = image:colorResolveAlpha(255, 255, 255, 50)
	local red = image:colorResolve(255, 0, 0)
	local green = image:colorResolve(0, 255, 0)
	for ix = 0, 0, 1 do
		...
		for n, p in pt:points() do
			...
			--image:setPixel(pt[n].x, pt[n].y-i, white50)
		end
		for i = l1+1, #points-l1 do
			...
			image:line(pt[i].x, pt[i].y, pt[i].x, pt[i].y-c, white50)
		end
		...
		if ppv then
			...
			image:line(pt[dppv[mind][1]].x, pt[dppv[mind][1]].y, pt[dppv[mind][1]].x, pt[dppv[mind][1]].y-points[dppv[mind][1]], red)
			local bestcuts = {2,3}
			...
			local bestmid = bestcuts[2]
			bestmid = math.floor(bestmid)
			--table.insert(bestmids, bestmid)
			--canvas:line(bestmid+origin.x, 25, bestmid+origin.x, 255, darkgreen)
			image:line(pt[bestmid].x, pt[bestmid].y, pt[bestmid].x, pt[bestmid].y-points[bestmid], green)
			local bestmid = bestcuts[1]
			bestmid = math.floor(bestmid)
			image:line(pt[bestmid].x, pt[bestmid].y, pt[bestmid].x, pt[bestmid].y-points[bestmid], green)
	...
	image = resize(image, math.max(sizex, sizey))
	image:string(gd.FONT_SMALL, 2, 2, "OVERLAY DIFF 1/2", white)
	image:gifAnimAdd(output, true, 0, 0, 10, 2)
	print("frm :"..frm)
end
--[[
